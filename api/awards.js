// GET  /api/awards
//        -> { awards, updatedAt, source }. Public - the standings this returns are already visible
//           on the live public dashboard (see vendor/dashboard_js_full.txt's renderAwards()), so
//           there is nothing extra to protect by gating reads.
...
const { getCurrentAwards, setAwardSlot } = require("../lib/awardsStore");
...
async function handleGet(req, res) {
  ...
  try {
    const { awards, updatedAt, source } = await getCurrentAwards();
    res.status(200).json({ awards, updatedAt, source });
  } catch (err) {
    res.status(500).json({ error: (err && err.message) || "Unknown error" });
  }
}
