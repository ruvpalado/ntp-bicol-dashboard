...
// Stored as one flat map, { "<AREA>": "<datetime-local value>" }, separately from the awards board
// itself (a different lifecycle: set rarely, read on every dashboard load) so reading/writing one
// never risks corrupting the other. An area missing from the map means "not yet configured" for
// that area specifically - see vendor/dashboard_js_full.txt's isAreaActivated()/
// isAwardsActiveForCurrentPage() for how an entirely-empty map keeps the whole module visible by
// default (this feature's behavior before it existed), while a partially-filled map only gates the
// areas that actually have a date set.
...
async function getActivationDates() {
  if (blobConfigured()) {
    try {
      ...
      const msg = String((err && err.message) || err);
      if (!/not\s*found|404|no such/i.test(msg)) {
        console.error("awardsStore.getActivationDates: Blob read failed:", err);
      }
    }
  }
  return {};
...
/**
 * Sets (or, given a falsy value, clears) one area's activation date/time, leaving every other
 * area's value untouched.
...
async function setActivationDate(area, iso) {
  ...
  const areas = await getActivationDates();
  const value = (iso && String(iso).trim()) ? String(iso).trim() : null;
  if (value) areas[key] = value; else delete areas[key];
  const { put } = require("@vercel/blob");
  const blob = await put(ACTIVATION_PATHNAME, JSON.stringify(areas), {
    access: "private",
    contentType: "application/json",
    allowOverwrite: true,
    addRandomSuffix: false,
...
module.exports = {
  ...
  getActivationDates, setActivationDate, ACTIVATION_PATHNAME,
};
