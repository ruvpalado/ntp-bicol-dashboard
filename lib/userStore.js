...
// Physically removes a PENDING account request (pending_setup or pending_approval) from the store -
// the record is deleted outright rather than rejected, so the email is free to submit a brand-new
// request. Active/rejected/revoked accounts are never touched (use revoke/reject for those).
async function deleteUser(email) {
  ...
  if (!user) return { ok: false, reason: "not_found" };
  if (user.status !== "pending_approval" && user.status !== "pending_setup") return { ok: false, reason: "wrong_status" };
  const remaining = users.filter((u) => u.email !== e);
  await saveAllUsers(remaining);
  return { ok: true, user };
}
