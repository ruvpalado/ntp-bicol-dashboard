...
// A real province upload, even with years of accumulated TSR Cohort data, tops out at a few
// thousand rows per sheet. A sheet whose declared used-range (ws['!ref']) reaches far beyond its
// real data - almost always because a cell was clicked, scrolled to, or formatted far down/right at
// some point and Excel silently "remembers" that as part of the sheet's dimensions even though
// nothing is actually there - forces XLSX.utils.sheet_to_json() to materialize an array entry for
// EVERY row (and cell) in that whole declared range before this file's own trimTrailingBlankRows()
// ever gets a chance to cut the empty tail back down. For a genuinely bloated range (Excel's max is
// over a million rows) that one call alone can blow well past a serverless function's memory ceiling
// - this was the actual cause of a production upload OOM-crashing on one province's file while an
// identically-formatted file from another province uploaded fine (both looked like ordinary,
// modest-sized files; only one had ever had a cell touched far outside its real data).
// Checked BEFORE sheet_to_json runs, so a bloated file is rejected with a clear, actionable message
// instead of silently trying to allocate an enormous array and crashing with an opaque "out of
// memory" error the admin has no way to diagnose or fix themselves.
const MAX_SHEET_ROWS = 50000;
const MAX_SHEET_CELLS = 2000000;
function checkSheetRangeIsSane(ws, sheetName) {
  const ref = ws && ws["!ref"];
  if (!ref) return;
  let range;
  try { range = XLSX.utils.decode_range(ref); } catch (e) { return; }
  const rows = range.e.r - range.s.r + 1;
  const cols = range.e.c - range.s.c + 1;
  if (rows > MAX_SHEET_ROWS || rows * cols > MAX_SHEET_CELLS) {
    const err = new Error(
      `Sheet "${sheetName}" appears to have data extending to row ${range.e.r + 1} (${rows.toLocaleString()} rows), ` +
        "far more than a real upload should ever contain. This almost always happens when a cell far " +
        "below or to the right of the real data was accidentally clicked, formatted, or pasted into at " +
        "some point - Excel keeps that as part of the sheet's size even though it looks empty. " +
        "Open the file, select all rows/columns beyond your real data, delete them (not just clear " +
        "their contents), save, and re-upload."
    );
    err.code = "BAD_FORMAT";
    throw err;
  }
...
function parseUpload(buffer, filename, opts) {
  ...
  if (extension === ".csv") {
    ...
    checkSheetRangeIsSane(workbook.Sheets[first], first);
  ...
  for (const name of workbook.SheetNames) {
    checkSheetRangeIsSane(workbook.Sheets[name], name);
