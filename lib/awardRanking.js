...
// TB Preventive Treatment: the primary metric is coverage_pct (accomplish/target x100). The
// standings table shows the raw accomplish (enrolled) and target counts alongside the rate.
function tptTiebreak(node) {
  const t = (node && node.tpt) || null;
  return {
    tptAccomplish: t && typeof t.enrolled === "number" ? t.enrolled : null,
    tptTarget: t && typeof t.target === "number" ? t.target : null,
  };
}

// Case Notification: the primary metric is the CNR rate per 100k. The standings table additionally
// shows the raw Case Notified count (cnr.notified - CNR + MN-notified patients) and the Case to be
// Notified target (targets.ctbn, the annual formula-derived notification target) alongside the rate.
function cnrTiebreak(node) {
  const c = (node && node.cnr) || null;
  const t = (node && node.targets) || null;
  return {
    cnrNotified: c && typeof c.notified === "number" ? c.notified : null,
    cnrTarget: t && typeof t.ctbn === "number" ? t.ctbn : null,
  };
}
...
// Notification) by the metric, highest first. This is the PROVINCIAL view: restricted to the given
// province's own units (key prefix "F|<province>|" / "M|<province>|").
function unitCandidates(kpi, catDef, province, period, periodValue) {
  ...
  for (const key of Object.keys(nodes)) {
    ...
    const name = titleCase(rawName) + " (" + (LABEL[provKey] || titleCase(provKey)) + ")";
    const item = { key, name, value: v, province: provKey };
    if (catDef.tiebreak) {
      const tb = catDef.tiebreak(resolved);
      if (tb.cureValue !== undefined) item.cureValue = tb.cureValue;
      if (tb.bactCount !== undefined) item.bactCount = tb.bactCount;
      if (tb.tptAccomplish !== undefined) item.tptAccomplish = tb.tptAccomplish;
      if (tb.tptTarget !== undefined) item.tptTarget = tb.tptTarget;
      if (tb.cnrNotified !== undefined) item.cnrNotified = tb.cnrNotified;
      if (tb.cnrTarget !== undefined) item.cnrTarget = tb.cnrTarget;
    }
    out.push(item);
  }
...
// Rank PROVINCES (the "P|" nodes) by the category's metric, highest first. This is the REGIONAL
// view - the user selects Regional to rank provinces against each other by their own aggregated
// accomplishments (Treatment Success / CNR / TPT each roll up to province level in the pipeline).
function provinceCandidates(kpi, catDef, period, periodValue) {
  const nodes = (kpi && kpi.nodes) || {};
  const out = [];
  for (const key of Object.keys(nodes)) {
    if (!key.startsWith("P|")) continue;
    const provKey = key.slice(2);
    const resolved = periodNode(nodes[key], period, periodValue);
    const v = catDef.metric(resolved);
    if (v == null) continue;
    const item = { key, name: LABEL[provKey] || titleCase(provKey), value: v, province: provKey };
    if (catDef.tiebreak) {
      const tb = catDef.tiebreak(resolved);
      if (tb.cureValue !== undefined) item.cureValue = tb.cureValue;
      if (tb.bactCount !== undefined) item.bactCount = tb.bactCount;
...
function candidatesFor(kpi, categoryKey, scope, province, period, periodValue) {
  ...
  if (!catDef) return [];
  // "region" ranks the six provinces (+ Naga City) against each other by their own aggregated
  // accomplishments; "province" ranks the selected province's units - municipalities for Case
  // Notification, facilities for the TSR/TPT categories.
  if (scope === "region") return provinceCandidates(kpi, catDef, period, periodValue);
  if (!province) return [];
  return unitCandidates(kpi, catDef, province, period, periodValue);
}
